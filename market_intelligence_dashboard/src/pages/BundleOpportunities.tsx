...
export default function BundleOpportunities() {
  ...
  if (isLoading) {
    return (
      <div className="flex py-20 items-center justify-center flex-col gap-3">
        ...
        <p className="text-sm text-muted-foreground font-mono uppercase tracking-widest">Loading Top 5 Bundle Opportunities...</p>
      </div>
    );
  }
    ...
    return (
      <Card className="border-danger/20 bg-danger/5 mt-4">
        <CardContent className="p-8 flex flex-col items-center text-center">
          ...
          <h2 className="text-xl font-bold mb-2 font-serif text-danger">Top 5 Bundle Opportunities Unavailable</h2>
          <p className="text-danger/80 max-w-lg">{getEngineErrorMessage(data, 'Insufficient data to compute Top 5 Bundle Opportunities.')}</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className="mt-4 border-dashed">
        <CardContent className="p-10 flex flex-col items-center text-center text-muted-foreground">
          ...
          <p>No bundle opportunities found.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="mt-6 space-y-4">
      <div className="mb-6">
        <h2 className="text-2xl font-bold">Top 5 Bundle Opportunities</h2>
        <p className="text-muted-foreground">Identifies high-potential bundle combinations using complement relationships.</p>
      </div>
      
      <div className="grid grid-cols-1 gap-4">
        {top5.map((item: any, idx: number) => {
          const score = Number(item.bundle_score) || 0;
              ...
              <CardContent className="p-5 flex flex-col md:flex-row items-start gap-4">
                ...
                <div className="flex-1 space-y-1">
                  ...
                  <div className="mt-3 p-3 bg-muted/30 rounded-md text-sm border border-border/50">
                    ...
                    {item.reason || 'This pairing demonstrates high demand overlap and complement strength. Bundling these items can increase average order value and capture adjacent market demand.'}
                  </div>
                </div>
