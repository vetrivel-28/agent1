...
export default function BundleOpportunities() {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['bundle-opportunities', categoryKey],
    queryFn: () => api.getBundleOpportunities({ topN: 10, scope: categoryScope }),
  });
  ...
  const createOpportunityEvidence = (item: any, rank: number, filterContext?: any): EvidenceData => {
        ...
        {top_items.map((item: any, idx: number) => {
          ...
          return (
            <Card 
              ...
              onClick={() => setSelectedEvidence(createOpportunityEvidence(item, idx + 1, { active_filters: activeFilters, filtered_row_count: filteredData.length, total_row_count: items.length }))}
            >
