...
export default function SubstituteIntelligence() {
  ...
  if (isLoading) {
    return (
      <div className="flex py-20 items-center justify-center flex-col gap-3">
        ...
        <p className="text-sm text-muted-foreground font-mono uppercase tracking-widest">Loading Top 5 Substitute Threats...</p>
      </div>
    );
  }
    ...
    return (
      <Card className="border-danger/20 bg-danger/5 mt-4">
        <CardContent className="p-8 flex flex-col items-center text-center">
          ...
          <h2 className="text-xl font-bold mb-2 font-serif text-danger">Top 5 Substitute Threats Unavailable</h2>
          <p className="text-danger/80 max-w-lg">{getEngineErrorMessage(data, 'Insufficient data to compute Top 5 Substitute Threats.')}</p>
        </CardContent>
      </Card>
    );
  }
    ...
    return (
      <Card className="mt-4 border-dashed">
        <CardContent className="p-10 flex flex-col items-center text-center text-muted-foreground">
          ...
          <p>No substitute threats found.</p>
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <div className="mt-6 space-y-4">
      <div className="mb-6">
        <h2 className="text-2xl font-bold">Top 5 Substitute Threats</h2>
        <p className="text-muted-foreground">Identifies substitute products stealing demand from the target market.</p>
      </div>
